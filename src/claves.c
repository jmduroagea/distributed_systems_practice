    ...
    for (int i = 0; i < current_retire_count; i++) {
        ...
        if (safe_to_free) {
            ...
            if (retire_count < HP_RETIRE_THRESHOLD) {
                ...
                fprintf(stderr,
                        "[hp_scan] WARN: retire_list llena, puntero %p no liberado (memory leak).\n",
                        ptr);
            }
        }
    }
                                       ...
                                       uint64_t                    h)
{
    ...
    for (int offset = 0; offset < HOPSCOTCH_NEIGHBORHOOD; offset++) {
        ...
        if (atomic_compare_exchange_strong_explicit(
                &new_table->entries[candidate].data,
                &expected,
                t,
    ...
    for (int i = 0; i < STASH_SIZE; i++) {
                ...
                memory_order_relaxed))
        {
            ...
            atomic_fetch_add_explicit(&new_table->stash_count, 1, memory_order_relaxed);
...
void trigger_parallel_resize(void)
{
    ...
    if (load < LOAD_FACTOR_THRESHOLD
        && atomic_load_explicit(&global_table->stash_count,
                                memory_order_relaxed) < STASH_SIZE / 2)
    {
    ...
    if (!new_table) {
        fprintf(stderr, "[resize] OOM: no se pudo asignar new_table. Resize abortado.\n");
        pthread_rwlock_unlock(&global_stw_rwlock);
        return;
    }
    ...
    if (!new_table->entries) {
        fprintf(stderr, "[resize] OOM: no se pudo asignar entries. Resize abortado.\n");
        free(new_table);
        pthread_rwlock_unlock(&global_stw_rwlock);
        return;
    }
    ...
    atomic_store_explicit(&new_table->total_items,
                          atomic_load_explicit(&global_table->total_items,
                                               memory_order_relaxed),
                          memory_order_relaxed);
    ...
    int total_chunks     = 1;
    while (total_chunks < total_chunks_raw) total_chunks <<= 1;
...
int set_value(char *key, char *value1, int N_value2, float *V_value2, struct Paquete value3) { 
    ...
    if (strnlen(key, MAX_STR_LEN + 1) > MAX_STR_LEN || strnlen(value1, MAX_STR_LEN + 1) > MAX_STR_LEN) return -1; 
    ...
    while (retries < MAX_RETRIES) {
        ...
        neighborhood_write_lock(ideal_idx, global_table->capacity, &seg_idx, &seg_idx2);
        ...
        // CORRECCIÓN DOBLE-CHECK STASH: Comprobar también el stash bajo el lock de la vecindad
        if (!already_exists) {
...
int get_value(char *key, char *value1, int *N_value2, float *V_value2, struct Paquete *value3) { 
    ...
    uint32_t last_idx = (ideal_idx + HOPSCOTCH_NEIGHBORHOOD - 1) % global_table->capacity;
    ...
    } while (neighborhood_read_retry(ideal_idx, global_table->capacity, seq1, seq2));
